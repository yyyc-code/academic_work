...
"""
'''
This class is to generate stochastic process from different diffusion models. 
r = risk free rate / drift
s0 = initial price of underlying asset
...
class stochastic_process(object):
    ...
    def GBM_simulation(self,method):  #for GBM model, we can generate paths with 'Eular' methods or use explicit solution.
        if method == 'Eular':
            st = pd.DataFrame()
    ...
    def Bachelier_simulation(self): # simulate process from Bachelier model
        st = pd.DataFrame()
        st['t=0'] = self.s0*np.ones(self.N,dtype=int)
        for i in range(0,int(1/self.deltat)):
    ...
    def CEV_simulation(self,beta): #simulate process from CEV
        st = pd.DataFrame()
        st['t=0'] = self.s0*np.ones(self.N,dtype=int)
        for i in range(0,int(1/self.deltat )):
